...
# =====================================================================================
#  THEME — dark navy + white cards, matching the provided screen samples
# =====================================================================================
st.markdown("""
<style>
...
section[data-testid="stSidebar"] * { color: #E9E7FA !important; }
section[data-testid="stSidebar"] label { color: #E9E7FA !important; }

/* Headings / body text on the dark canvas */
h1, h2, h3, h4, h5, h6 { color: #FFFFFF !important; font-weight: 700 !important; }
p, span, label, .stMarkdown, .stCaption { color: #D8D6EE; }
...
div[data-testid="stVerticalBlockBorderWrapper"] h3 { color: #14123A !important; }
...
.stButton > button:hover { background: #3B30C0; color: #FFFFFF; }
.stButton > button[kind="primary"] { background: #4B3FE4; }
...
div[data-testid="stMetric"] label, div[data-testid="stMetric"] div { color: #14123A !important; }

/* Tabs */
.stTabs [data-baseweb="tab"] { color: #D8D6EE; font-weight: 600; }
.stTabs [aria-selected="true"] { color: #FFFFFF !important; border-bottom-color: #4B3FE4 !important; }
</style>
...
def signup_page():
    ...
    with mid:
        with st.container(border=True):
            st.markdown(f"<h2 style='text-align:center;'>\U0001F4D8 {APP_NAME}</h2>", unsafe_allow_html=True)
            st.markdown("<p style='text-align:center; margin-top:-10px;'>Professional Worksheet Generator</p>", unsafe_allow_html=True)
...
def admin_panel_page():
    ...
    if not users:
        ...
        for username, email, verified, is_admin in users:
            with st.expander(
                f"{'\U0001F451 ' if is_admin else ''}{username}  —  {email}"
...
def render_question_bank_page():
    ...
    with st.container(border=True):
        st.markdown("""
**How it works:**
Upload a CSV or Excel file with your own questions using these columns:

`grade, subject, topic, q_type, text, options, match_left, match_right, answer, image_keyword`

- Use `|` to separate multiple values in a cell (e.g. `Cat|Dog|Cow|Horse`)
- Matches are filtered by the Grade / Subject / Topic / Question Types you choose in **Create Worksheet**

To actually build a worksheet from your question bank, go to
**\u270D\ufe0f Create Worksheet \u2192 Method 3 — Your Own Question Bank** tab.
""")
    st.download_button(
...
def render_cover_designer_page():
    ...
    st.caption("Your worksheet's branding — school name, logo, and color theme.")
    with st.container(border=True):
        st.markdown("""
Branding options (School Name, School Logo, Cover Color Theme, and the Auto / Colorful / Professional
visual theme) are set together with the rest of your worksheet settings inside:

**\u270D\ufe0f Create Worksheet \u2192 Worksheet Settings** (top of that page)

Setting them there keeps everything — branding, grade, subject, and question types — consistent
for the worksheet you're about to generate.
""")
        if st.button("Go to Create Worksheet \u2192", type="primary"):
            st.info("Use the **Create Worksheet** item in the sidebar to continue.")


def render_school_settings_page():
    st.title("\u2699\ufe0f School Settings")
    st.caption("Fields shown on the worksheet, spacing, binding, and copies.")
    with st.container(border=True):
        st.markdown("""
These settings (toggling Student Name / Class / Roll No. / Date, spacing between questions,
binding gap, number of copies, and copy mode) live inside:

**\u270D\ufe0f Create Worksheet \u2192 Worksheet Settings** (top of that page)

They apply automatically to whichever method (1, 2, or 3) you use to generate your worksheet.
""")
        if st.button("Go to Create Worksheet", type="primary"):
            st.info("Use the **Create Worksheet** item in the sidebar to continue.")
