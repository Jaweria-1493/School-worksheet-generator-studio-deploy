...
def create_user(username: str, email: str, password: str):
    ...
    try:
        conn.execute(
            "INSERT INTO users (username, email, password_hash, salt, verified, is_admin, approved) VALUES (?, ?, ?, ?, 0, ?, ?)",
            (username.strip(), email.strip().lower(), ph, salt, is_admin, approved),
        )
        conn.commit()
...
def admin_panel_page():
    ...
    with st.expander("\U0001F4CB Full user list (all info at a glance)", expanded=True):
        st.dataframe(
            [
                {
                    "Username": u, "Email": e, "Verified": bool(v),
                    "Approved": bool(a), "Admin": bool(ad),
                }
