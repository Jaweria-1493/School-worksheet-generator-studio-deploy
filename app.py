...
def render_dashboard():
    ...
    if _my_logs:
        with st.expander(f"\U0001F4C4 Recent Worksheets ({min(5, len(_my_logs))} shown)", expanded=False):
            for u, t, g, subj, topic, fmt, nq, nc in _my_logs[:5]:
                st.markdown(f"- **{subj} — {topic}** (Grade {g}) · {fmt} · {nq} questions · {t[:16].replace('T', ' ')} UTC")
